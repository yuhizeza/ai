...
def detect_cause_effect(sentence: str) -> bool:
    s = normalize(sentence)
    causes = {"때문에", "그러므로", "따라서", "그래서", "결과적으로", "므로", "해서", "하여"}
    results = {
...
SETS = {
    "1세트: 사회적 촉진·억제": {
        ...
        "passage": """
**기자:** 심리학 용어인 ‘사회적 촉진’과 ‘사회적 억제’를 학습에 어떻게 적용할 수 있을까요?

**전문가:** 비교적 쉬운 취미 생활이나 큰 노력을 들일 필요가 없는 과제는 집에서 혼자 하는 것보다 커피숍이나 도서관에서 하거나, 친숙하고 좋아하는 과목이라면 공부 모임을 만들어 다른 사람들과 함께 공부하는 것이 더 효율적일 수 있습니다.
...
""",
    ...
    "2세트: 정전기": {
        ...
        "passage": """
정전기란 전하가 정지 상태로 있어 그 분포가 시간적으로 변화하지 않는 전기와 그로 인한 전기 현상을 말합니다. 쉽게 말하면 흐르지 않고 머물러 있는 전기입니다.

우리가 실생활에서 쓰는 전기가 ‘흐르는 물’이라면, 정전기는 ‘높은 곳에 고여 있는 물’이라고 할 수 있습니다. 정전기의 전압은 매우 높지만 전하가 이동하지 않고 머물러 있어 위험하지는 않습니다.
""",
        "q1": {
...
""",
        "q1": {
            ...
            "guide": [
                "㉠: 인공 지능 그림과 대응되는 로봇 경기의 모습을 씁니다.",
                "㉡: 예술로 보기 어려운 근거와 결론을 함께 씁니다.",
                "㉢: 인공 지능 그림이 지닌 가치나 의미를 씁니다.",
            ],
            "rules": {
                "㉠": {
                    "required_groups": [
...
def grade_q2(answer1: str, answer2: str, rule: Dict) -> GradeResult:
    ...
    for idx, answer in enumerate(answers, start=1):
        ...
        if declared:
            if declared not in detected:
                return GradeResult(
                    ...
                    0,
                    4,
                    [f"({idx}) 괄호 속 ‘{declared}’와 실제 문장 방식이 일치하지 않습니다. 감지된 방식: {', '.join(sorted(detected)) or '없음'}"],
                )
        ...
        ok, sub_reasons = passage_based(answer, rule["passage_groups"], rule["contradictions"])
        reasons.extend([f"({idx}) {r}" for r in sub_reasons])
        if not ok:
...
def render_passage(content: Dict) -> None:
    st.markdown(f"### 📖 읽을 글: {content['passage_title']}")
    with st.container(border=True):
        st.markdown(content["passage"])


def render_conditions(items: List[str]) -> None:
    with st.container(border=True):
        st.markdown("#### ✅ 작성 조건")
        for item in items:
            st.markdown(f"- {item}")
...
def render_result(title: str, result: GradeResult, model: Optional[str] = None) -> None:
    with st.container(border=True):
        if result.passed:
...
st.caption("글과 문제를 읽고 답을 작성하세요. 동의어·유사 표현은 인정하지만 오개념과 결론 방향 오류는 잡아냅니다.")
...
with st.sidebar:
    ...
    st.info("자동 채점은 1차 판단입니다. 창의적인 답안은 교사가 최종 확인해야 합니다.")

content = SETS[selected_set]
render_passage(content)
st.divider()

if selected_type.startswith("서·논술형 1"):
    q = content["q1"]
    ...
    for col, blank in zip(cols, ["㉠", "㉡", "㉢"]):
        with col:
            answers[blank] = st.text_area(
                f"{blank} 답안",
                height=130,
                placeholder=f"{blank}에 들어갈 내용을 쓰세요.",
                key=f"{selected_set}_{selected_type}_{blank}",
            )
    ...
    if st.button("채점하기", type="primary", use_container_width=True):
        ...
        for blank in ["㉠", "㉡", "㉢"]:
            ...
            total += result.score
            render_result(blank, result, q["rules"][blank]["model"])
        st.metric("총점", f"{total:.0f} / 3")

elif selected_type.startswith("서·논술형 2"):
    q = content["q2"]
    st.markdown(f"## {q['title']}")
    st.info(q["prompt"])
    ...
    with st.expander("설명 방법 6가지 확인하기"):
        st.table([
            {"방법": "정의", "기준": "대상의 뜻이나 개념을 밝힘"},
            {"방법": "예시", "기준": "구체적인 사례를 제시함"},
            {"방법": "인과", "기준": "원인과 결과를 연결함"},
            {"방법": "분석", "기준": "대상을 둘 이상의 요소로 나눔"},
            {"방법": "비교와 대조", "기준": "두 대상의 공통점이나 차이점을 밝힘"},
            {"방법": "분류와 구분", "기준": "기준에 따라 종류를 나눔"},
        ])
    ...
    with c1:
        answer1 = st.text_area(
            "(1) 첫 번째 문장",
            height=165,
            placeholder="첫 번째 설명 방법을 사용해 문장을 쓰세요.",
            key=f"{selected_set}_{selected_type}_1",
        )
    with c2:
        answer2 = st.text_area(
            "(2) 두 번째 문장",
            height=165,
            placeholder="첫 문장과 다른 설명 방법으로 쓰세요.",
            key=f"{selected_set}_{selected_type}_2",
        )
    ...
    with right:
        ...
        audio_element = st.text_area(
            ...
            key=f"{selected_set}_{selected_type}_ae",
        )
        audio_effect = st.text_area(
            "이 청각 요소는 어떤 내용을 전달하나요?",
            height=155,
            placeholder="앞에서 쓴 소리가 지문의 어떤 내용을 강조하는지 설명하세요.",
            key=f"{selected_set}_{selected_type}_afx",
        )

    with st.expander("6점 배점 기준 확인하기"):
        st.table([
            {"평가 요소": "시각 요소", "배점": 1},
            {"평가 요소": "시각 효과–요소 연결", "배점": 1},
            {"평가 요소": "시각 효과–본문 근거", "배점": 1},
            {"평가 요소": "청각 요소", "배점": 1},
            {"평가 요소": "청각 효과–요소 연결", "배점": 1},
            {"평가 요소": "청각 효과–본문 근거", "배점": 1},
        ])
